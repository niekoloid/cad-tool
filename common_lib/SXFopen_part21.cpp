/**
	...
	２．インターフェース
	int SXFopen_part21(char IN_file_name[257],
						int IN_rw_flg, 
...
__declspec(dllexport) int WINAPI SXFopen_part21(char IN_file_name[], 
												int IN_rw_flg, 
												double IN_tolerance1, 
												double IN_tolerance2, 
												double IN_divide, 
												...
												char IN_author[], 
												char IN_org[], 
												char IN_translator_name[], 
												char OUT_version[])
{
	//変換管理オブジェクト生成
	gl_Man = new SXFManageC();
	int ret = gl_Man->Open(IN_file_name,
							IN_rw_flg,
							IN_tolerance1,
							IN_tolerance2,
							IN_divide,
...
__declspec(dllexport) int WINAPI SXFset_file_version_part21(char IN_file_version[])
{
	gl_FileVersion = IN_file_version;
	return 0;
}

__declspec(dllexport) int WINAPI SXFget_file_version_part21(char OUT_file_version[])
{

/**
	...
	SXFutyConvKanjiC.cpp						çÏê¨ì˙ÅF2000.04.17

	ÇPÅDÉNÉâÉXäTóv
		JISÉRÅ[ÉhÅ©Å®UNICODEÇÃïœä∑

	ÇQÅDóöó
		óöóî‘çÜ	ì˙ït		íSìñé“		äTóv
		çÏê¨		2000.04.17	ã{âÄ(FQS)	Ç»ÇµÅB
		çXêV		2002.02.20	K.Naono		â¸èCëŒâû
		çXêV		2002.02.20	K.Naono		â¸èCëŒâû
**/
#include "stdafx.h"
#include <afx.h>
...
/**
	...
	ÇQÅDÉCÉìÉ^Å[ÉtÉFÉCÉX
		void	 SXFutyConvKanjiC::SXFutyConvKanjiC()
		
	à¯êî	ÅF	å^		à¯êîñº				ì‡óe
		Ç»Çµ
...
/**
	...
	à¯êî	ÅF	å^		à¯êîñº		ì‡óe
				char*	Direction	ïœä∑ï˚å¸
									FromJISÅF	JISÅ®UNICODE
									ToJIS:		UNICODEÅ®JIS
				char*	InChar		ïœä∑ëOï∂éöóÒ
				char*	OutChar		ïœä∑å„ï∂éöóÒ
				size_t	OutCharSize	ïœä∑å„ï∂éöóÒÇéÛÇØéÊÇÈÉoÉbÉtÉ@ÉTÉCÉY

	ÇRÅDóöó
		óöóî‘çÜ	ì˙ït		íSìñé“			äTóv
		çÏê¨		2000.04.17	ã{âÄ(FQS)		Ç»ÇµÅB
		çXêV		2015.04.27	T.Matsushima	JPCERT/CC éwìE ê∆é„ê´ëŒâû
**/
//int SXFutyConvKanjiC::ConvKanji(char *Direction, char *InChar, char *OutChar)
int SXFutyConvKanjiC::ConvKanji(char *Direction, char *InChar, char *OutChar,size_t OutCharSize)
{
	short n1,n2;
	short n3;
	int   i,p1,q1,r1,OutCode,kanji,rtn,TableKind,KanjiTable2[1344];
	size_t num,count;	// 64bitëŒâû
	char  buf[5],pp[3],*dmy;
	...
	int KanjiTable[] = {
	/* Å@      ÅA      ÅB      ÅC      ÅD      ÅE      ÅF      ÅG     */
	 0x3000, 0x3001, 0x3002, 0xFF0C, 0xFF0E, 0x30FB, 0xFF1A, 0xFF1B, 
	/* ÅH      ÅI      ÅJ      ÅK      ÅL      ÅM      ÅN      ÅO     */
	 0xFF1F, 0xFF01, 0x309B, 0x309C, 0x00B4, 0xFF40, 0x00A8, 0xFF3E, 
	/* ÅP      ÅQ      ÅR      ÅS      ÅT      ÅU      ÅV      ÅW     */
	...
	/* Å@      Å@      Å@      Å@      Å@      Å@      Å@      Å@     */
	 0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 
	/* á@      áA      áB      áC      áD      áE      áF      áG     */
	 0x2460, 0x2461, 0x2462, 0x2463, 0x2464, 0x2465, 0x2466, 0x2467, 
	/* áH      áI      áJ      áK      áL      áM      áN      áO     */
	 0x2468, 0x2469, 0x246A, 0x246B, 0x246C, 0x246D, 0x246E, 0x246F, 
	/* áP      áQ      áR      áS      áT      áU      áV      áW     */
	...
	/* ò¯      ò˘      ò˙      ò˚      ò¸                             */
	 0x5080, 0x509A, 0x5085, 0x50B4, 0x50B2, 0x0000//èIí[
	};
	...
	/* ¸H      ¸I      ¸J      ¸K                                     */
	 0x9D6B, 0xFA2D, 0x9E19, 0x9ED1, 0x0000//èIí[
	};

	if (strncmp(Direction,"FromJIS",7)==0){
		num = strlen(InChar);
		...
		while (count < num){
			...
			if (n1 < 0x80 ){
				if (kanji == 1){
					//	JPCERT/CC éwìE ê∆é„ê´ëŒâû	T.Matsushima	2015.04.27
...
//	ADD(S)	â¸èCëŒâû	K.Naono		02.02.20
				else if(n1 == 0x005C)
				{
					//	JPCERT/CC éwìE ê∆é„ê´ëŒâû	T.Matsushima	2015.04.27
					//strncat(OutChar,"\\\\",2);
...
//	ADD(E)	â¸èCëŒâû	K.Naono		02.02.20
				else{
					//	JPCERT/CC éwìE ê∆é„ê´ëŒâû	T.Matsushima	2015.04.27
				...
				//	MOD(S)	JPCERT/CC éwìE ê∆é„ê´ëŒâû	T.Matsushima	2015.04.27
					//sprintf(buf,"%4.4X",OutCode);
					//strcat(OutChar,buf);
					sprintf_s(buf,_countof(buf),"%4.4X",OutCode);
					strcat_s(OutChar,OutCharSize,buf);
				//	MOD(E)	JPCERT/CC éwìE ê∆é„ê´ëŒâû	T.Matsushima	2015.04.27
					count++;
				}
			}
	...
	else if (strncmp(Direction,"ToJIS",5) == 0){
...
/**
	...
	ÇQÅDÉCÉìÉ^Å[ÉtÉFÉCÉX
		void	 AddTable(int KanjiTable[])

	à¯êî	ÅF	å^		à¯êîñº			ì‡óe
				int		KanjiTable[]	äøéöÉeÅ[ÉuÉã

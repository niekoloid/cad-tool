...
void main(){
	...
	if (rw_flg == 0){
		...
		if (SXFget_file_version_part21(file_version) < 0)
		{
			return ;
		}
		/** ファイル作成者 **/
		...
		EndFlag = 0;
		while(!EndFlag){
			int ret_layer =SXFread_table(1,&las,&EndFlag);
			if (ret_layer > 0){
				/****** ここに要素の処理を記載 ******/
		...
		EndFlag = 0;
		while(!EndFlag){
			int ret_precolor = SXFread_table(2,&pcs,&EndFlag);
			if (ret_precolor > 0){
				/****** ここに要素の処理を記載 ******/
		...
		EndFlag = 0;
		while(!EndFlag){
			int ret_usercolor = SXFread_table(3,&ucs,&EndFlag);
			if (ret_usercolor > 0){
				/****** ここに要素の処理を記載 ******/
		...
		EndFlag = 0;
		while(!EndFlag){
			int ret_pretype = SXFread_table(4,&pls,&EndFlag);
			if (ret_pretype > 0){
				/****** ここに要素の処理を記載 ******/
		...
		EndFlag = 0;
		while(!EndFlag){
			int ret_usertype = SXFread_table(5,&uls,&EndFlag);
			if (ret_usertype > 0){
				/****** ここに要素の処理を記載 ******/
		...
		EndFlag = 0;
		while(!EndFlag){
			int ret_width = SXFread_table(6,&lws,&EndFlag);
			if (ret_width > 0){
				/****** ここに要素の処理を記載 ******/
		...
		EndFlag = 0;
		while(!EndFlag){
			int ret_font = SXFread_table(7,&tfs,&EndFlag);
			if (ret_font > 0){
				/****** ここに要素の処理を記載 ******/
		...
		while(!EndFlag){ 
			...
			while(!FeatureEndFlag){
				...
				if (error_code > 0){
					...
					else if (!strcmp(FeatureName, "CURVE_DIMENSION")){//角度寸法読みこみ
						/****** ここに要素の処理を記載 ******/
						*dump_file << "--------------------CURVE_DIMENSION-----------------------" << endl;
						*dump_file << "layer = " << ((CurveDim_Struct*)Struct)->layer << endl;
						*dump_file << "color = " << ((CurveDim_Struct*)Struct)->color << endl;
	...
	/** 読み書きフラグが1の場合、Write処理 **/
		...
		if (SXFopen_part21(out_file_name,		/** 出力ﾌｧｲﾙ名 **/
		...
		layer_str->lflag = 1;
		/** レイヤコード出力 **/
		return_code = SXFwrite_table(1,layer_str);
		/********************************/
		/** レイヤコード出力データ設定 **/
		/********************************/
		/****** ここに要素の処理を記載 ******/
		...
		/** 既定義色出力 **/
		return_code = SXFwrite_table(2,predefcolor_str);
		/****************************/
		/** 既定義色出力データ設定 **/
		/****************************/
		/****** ここに要素の処理を記載 ******/
		...
		/** ユーザ定義色出力 **/
		return_code = SXFwrite_table(3,userdefcolor_str);
		/********************************/
		/** ユーザ定義色出力データ設定 **/
		/********************************/
		/****** ここに要素の処理を記載 ******/
		...
		/** 既定義線種出力 **/
		return_code = SXFwrite_table(4,predeffont_str);
		/******************************/
		/** 既定義線種出力データ設定 **/
		/******************************/
		/****** ここに要素の処理を記載 ******/
		...
		/** ユーザ定義線種出力 **/
		return_code = SXFwrite_table(5,userdeffont_str);
		/**********************************/
		/** ユーザ定義線種出力データ設定 **/
		/**********************************/
		/****** ここに要素の処理を記載 ******/
		...
		/** 線幅出力 **/
		return_code = SXFwrite_table(6,width_str);
		/************************/
		/** 線幅出力データ設定 **/
		/************************/
		/****** ここに要素の処理を記載 ******/
		...
		/** 文字フォント出力 **/
		return_code = SXFwrite_table(7,textfont_str);
		/********************************/
		/** 文字フォント出力データ設定 **/
		/********************************/
		/****** ここに要素の処理を記載 ******/
		...
		/** 図面表題欄出力 **/
		return_code = SXFwrite_next_feature("ATTRIBUTE",Attribute_str);
		...
		Polyline_str->y.Add(d);
		/** 折線出力 **/
		return_code = SXFwrite_next_feature("POLYLINE",Polyline_str);
		...
		Ccurve_str->flag = 1;
		/** 複合曲線出力 **/
		int curve2 = SXFwrite_assembly_name(3,Ccurve_str);
		...
		Sfigorg_str->flag = 4;
		/** 複合図形出力 **/
		return_code =SXFwrite_assembly_name(2,Sfigorg_str);
		...
		/** 楕円弧出力 **/
		return_code = SXFwrite_next_feature("ELLIPSE_ARC",Ellipse_Arc_str);
		...
		Text_str->direct = 1;
		/** 文字要素出力 **/
		return_code = SXFwrite_next_feature("TEXT",Text_str);
		...
		/** 楕円出力 **/
		return_code = SXFwrite_next_feature("ELLIPSE",Ellipse_str);
		/** 複合図形配置データ設定 **/
		/****** ここに要素の処理を記載 ******/
		Sfigloc_str->layer = 1;
		...
		/** 点マーカ出力 **/
		return_code = SXFwrite_next_feature("POINT_MARKER",PointMarker_str);
		...
		Spline_str->y.Add(d);
		/** スプライン出力 **/
		return_code = SXFwrite_next_feature("SPLINE",Spline_str);
		...
		/** クロソイド出力 **/
		return_code = SXFwrite_next_feature("CLOTHOID",Clothoid_str);
		...
		/** 直線寸法出力 **/
		return_code = SXFwrite_next_feature("LINEAR_DIMENSION",LinearDim_str);
		...
		/** 弧長寸法出力 **/
		return_code = SXFwrite_next_feature("CURVE_DIMENSION",CurveDim_str);
		...
		/** 角度寸法出力 **/
		return_code = SXFwrite_next_feature("ANGULAR_DIMENSION",AngularDim_str);
		/** 直径寸法データ設定 **/
		/****** ここに要素の処理を記載 ******/
		...
		/** 直径寸法出力 **/
		return_code = SXFwrite_next_feature("DIAMETER_DIMENSION",DiameterDim_str);
		/** 半径寸法データ設定 **/
		/****** ここに要素の処理を記載 ******/
		...
		/* 半径寸法出力 */
		return_code = SXFwrite_next_feature("RADIUS_DIMENSION",RadiusDim_str);
		/** 引出し線データ設定 **/
		/****** ここに要素の処理を記載 ******/
		Label_str->layer = 1;
		Label_str->color = 16;
		...
		Label_str->direct = 1;
		/** 引出し線出力 **/
		return_code = SXFwrite_next_feature("LABEL",Label_str);
		/** バルーンデータ設定 **/
		/****** ここに要素の処理を記載 ******/
		Balloon_str->layer = 1;
		...
		/** バルーン出力 **/
		return_code = SXFwrite_next_feature("BALLOON",Balloon_str);
		/** 既定義シンボルデータ設定 **/
		/****** ここに要素の処理を記載 ******/
		...
		/** 外部定義ハッチ出力 **/
		return_code = SXFwrite_next_feature("EXTERNALLY_DEFINED_HATCH",ExHatch_str);
		...
		/** ハッチング(塗り)出力 **/
		return_code = SXFwrite_next_feature("FILL_AREA_STYLE_COLOUR",FillColour_str);
		...
		/** ハッチング(パターン)出力 **/
		return_code = SXFwrite_next_feature("FILL_AREA_STYLE_TILES",FillTiles_str);
		...
		/** 円出力 **/
		return_code = SXFwrite_next_feature("CIRCLE",Circle_str);
		/** 複合図形配置データ設定 **/
		/****** ここに要素の処理を記載 ******/
		Sfigloc_str->layer = 0;

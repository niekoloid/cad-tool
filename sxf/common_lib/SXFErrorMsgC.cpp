...
/**
	...
	履歴番号		日付け				担当者				概要
	作成			2000.3.27			宮園(FQS)			なし。
	更新			2015.04.27			T.Matsushima		SXFPopMsg()が仕様書通りに動作していないので修正

**/
void	SXFErrorMsgC::RecordError(SXFErrorCode IN_Code, const char* IN_Name, int IN_param, ...)
{
//	char strmsg[5000];	Move to SXFPopMsg.cpp		SXFPopMsg()修正
	va_list args;
	/* 引数リストへのポインタ */
...
//	MOD(S)	SXFPopMsg()修正	T.Matsushima	2015.04.27
	//vsprintf(strmsg, erec->GetText(), args);
	//erec->SetText(strmsg);
	erec->MakeErrorMessage(args);
//	MOD(E)	SXFPopMsg()修正	T.Matsushima	2015.04.27

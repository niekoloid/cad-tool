/**
	...
	SXFFillColour_checkC.cpp								作成日：2000.4.3
										  	...
										  	SXFComInfoC* IN_Info)
{
	...
	m_feature_name = "FILL_AREA_STYLE_COLOUR";
	...
	if (RW_Flag == 0){
		/** 属性以外のルールチェック **/
		ret = ReadFeature_Check(IN_fillcolour_str);
	}
	//WRITE
	else if (RW_Flag == 1){
		/** 属性以外のルールチェック **/
...
**/
int SXFFillColour_checkC::ReadFeature_Check(Fill_area_style_colour_Struct *IN_Struct)
{
	//外形のフィーチャコードが定義されていない値のとき

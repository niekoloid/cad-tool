/**
	...
	SXFFillHatching_checkC.cpp								作成日：2000.4.3
											  	...
											  	SXFComInfoC* IN_Info)
{
	...
	m_feature_name = "FILL_AREA_STYLE_HATCHING";
	...
	//READ
	if (RW_Flag == 0){
		ret = ReadFeature_Check(IN_fillhatching_str);
	}
	//WRITE
	else if (RW_Flag == 1){
...
**/
int SXFFillHatching_checkC::ReadFeature_Check(Fill_area_style_hatching_Struct *IN_Struct)
{
	//外形のフィーチャコードが定義されていない値のとき
	if (!m_ReadFeatureManager->CheckHatchID(IN_Struct->out_id)){
		SXFErrorMsgC::RecordError(SXFERROROUTID,
									m_feature_name,
									3,
									m_InstanceID,
	...
	for (int i = 0; i < IN_Struct->number; i++){
		...
		//中抜きのフィーチャコードが定義されていない値のとき
		if (!m_ReadFeatureManager->CheckHatchID(IN_Struct->in_id[i])){
			SXFErrorMsgC::RecordError(SXFERRORINID,
									m_feature_name,
									3,
									m_InstanceID,

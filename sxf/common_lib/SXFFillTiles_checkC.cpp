											  	...
											  	SXFComInfoC* IN_Info)
{
	...
	m_feature_name = "FILL_AREA_STYLE_TILES";
	...
	if (RW_Flag == 0){
		/** 属性以外のルールチェック **/
		ret = ReadFeature_Check(IN_filltiles_str);
	}
	//WRITE
	else if (RW_Flag == 1){
		/** 属性以外のルールチェック **/
...
**/
int SXFFillTiles_checkC::ReadFeature_Check(Fill_area_style_tiles_Struct *IN_filltiles_str)
{
	//ハッチパターンの繰り返しベクトル1の大きさが範囲外の値のとき
	if (double_check(IN_filltiles_str->hatch_pattern_vector1) < 0){
		SXFErrorMsgC::RecordError(SXFERRORVECTOR,
	...
	//外形のフィーチャコードが定義されていない値のとき
	if (!m_ReadFeatureManager->CheckHatchID(IN_filltiles_str->out_id)){
		SXFErrorMsgC::RecordError(SXFERROROUTID,
									m_feature_name,
									3,
									m_InstanceID,
	...
	for (int i = 0; i < IN_filltiles_str->number; i++){
		...
		//中抜きのフィーチャコードが定義されていない値のとき
		if (!m_ReadFeatureManager->CheckHatchID(IN_filltiles_str->in_id[i])){
			SXFErrorMsgC::RecordError(SXFERRORINID,
										m_feature_name,
										3,
										m_InstanceID,
